...
//local
import type { InkPlugin, TemplateServers } from './types';
...
export default function plugin(server: Server) {
  ...
  server.on('config', req => {
    ...
    server.register('ink', { compiler, servers, render });
  ...
  server.on('route', req => {
    const server = req.context;
    //get server environment
    const mode = server.config.path('template.config.dev.mode', 'http');
    const environment = server.config.path('server.mode', 'production');
    const development = environment !== 'production';
    //dont add dev routes if not in development mode
    if (!development) return;
    const { compiler, servers } = server.plugin<InkPlugin>('ink');
